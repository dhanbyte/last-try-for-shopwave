    ...
    try {
        ...
        if (dbConnect && Product) {
            try {
                ...
                // Combine: Database products first, then vendor products, then JSON products for fashion only
                let allProducts = [...transformedRegularProducts, ...transformedVendorProducts];
                
                // Add JSON products only for fashion category
                const fashionProducts = FASHION_PRODUCTS.map(p => ({
                    ...p,
                    id: p.id,
                    isVendorProduct: false,
                    inStock: true
                ...
                allProducts = [...allProducts, ...fashionProducts];
        ...
        // Return only fashion products from JSON if database fails
        console.log('Using JSON fashion products only - database connection failed');
        
        // Only return fashion products from JSON files
        const allProducts = FASHION_PRODUCTS.map(p => ({
            ...p,
            isVendorProduct: false,
            inStock: true
        }));
        ...
        // Return empty array or only fashion products as fallback
        const fashionFallback = fallbackProducts.filter(p => p.category === 'Fashion');
        return NextResponse.json(fashionFallback.map(p => ({
            ...p,
            id: p._id,
