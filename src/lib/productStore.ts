    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (response.ok) {
          ...
          // Add fashion products from JSON and customizable products
          ...
          const allProducts = [...processedProducts, ...fashionProducts, ...CUSTOMIZABLE_PRODUCTS];
        ...
        if (error.name !== 'AbortError') {
          console.warn('API load failed, using fallback products');
        }
      }
    }

    // Fallback to JSON products if API fails or during SSR (only fashion products)
    const fashionProducts = FASHION_PRODUCTS.map(product => ({
      ...product,
      slug: product.slug || product.id || product.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'product'
    }));
    
    const fallbackProducts = [...fashionProducts, ...CUSTOMIZABLE_PRODUCTS];
